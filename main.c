...
void jump_next_favorite(void)
{
	if (n_favorite_channels)
	{
		...
		for(attempts=0; attempts<n_favorite_channels; attempts++)
		{
			...
			{
				change_channel(s_i, c_i, TRUE, TRUE);
				break;
			}
...
void redraw()
{
	...
	change_channel(current_server, current_server_channel_nr, TRUE, FALSE);
...
void find_next_channel_with_data(BOOL me_only, BOOL forwards)
{
	...
	else
	{
		...
		if (jumpy_navigation)
			change_channel(check_server, check_channel, TRUE, TRUE);
		else
		{
			...
			change_channel(check_server, check_channel, FALSE, TRUE);
		}
	}
}
	...
	for(;;)
	{
		...
		else if (c == 20)	/* ^T */
		{
			...
			if (prev.s != -1 && prev.c != -1)
				change_channel(prev.s, prev.c, TRUE, FALSE);
			else
				wrong_key();
		}
		else if (c == 17)	/* ^Q */
			...
			if (utf8_strlen(cur_channel() -> input) > 0)
			{
				...
				if (do_line)
				{
					...
					if (user_command(current_server, current_server_channel_nr, ul_asc, do_commands) == -1)
					{
						...
						close(cur_server() -> fd);
						update_statusline(current_server, current_server_channel_nr, "Connection to %s:%d closed (6)", cur_server() -> server_host, cur_server() -> server_port);
					}
					else
					{
						add_to_buffer(cur_channel() -> input_buffer, ul_asc, cur_server() -> nickname, FALSE, current_server, current_server_channel_nr);
